...
- **cmd_planar**: (numpy.array (2)) get/set planar desired coordinates. Only when planar_control is ideal or step.
...
- **NNDetector** (dictionary) detections obdtained since last detection event. each key is associate to the name of the detected agent, and the item associated is a list of [distance, relative horizontal angle, relative vertical angle]. A key named time_lapsed contains the value of the time interval since last detection.

**Methods**
- **cmd_fhd(force,heading,depth)**->None: Set desired command as force (float or array-like(2)), heading (float), depth (float). 